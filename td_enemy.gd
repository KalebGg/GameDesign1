...
func _physics_process(delta):
	...
	if int(AI_STATE) >= 1 and int(AI_STATE) <= 8:
		...
		raycastM.taget_position = raydir * vision_distance
		raycastL.target_position = \
			raydir.rotation(deg_to_rad(-45)).normalized() * vision_distance 
	...
	if animation_lock == 0.0:
		...
		velocity += inertia
		move_and_slide()
		inertia = inertia.movr_toward(Vector2(), delta * 1000.0)
	pass
