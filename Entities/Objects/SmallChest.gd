...
func bounce_towards_player(item, player):
	var direction = (player.global.postition - global_position).normalized()
	...
	tween.tween_property(item, "global_postition", item.global_position + bounce_path, 0.5)
	tween.play()
...
func drop_item(item_name, value, player):
	...
	item.global_postition = self.global_position
